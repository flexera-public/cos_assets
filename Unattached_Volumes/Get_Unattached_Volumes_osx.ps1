...
foreach ($account in $accounts) {
    ...
    $total_unattached += $unattached | Select-Object @{name="accountnumber";expression={$account}},name,volume_type_href,iops,description,resource_uid,size,status,created_at,updated_at,cloud_specific_attributes,@{name="href";expression={$($_.links | Where-Object rel -eq "self").href}} #| Export-Csv "./$account-unattached-volumes.csv"  
}
