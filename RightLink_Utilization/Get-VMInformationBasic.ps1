    ...
    if (!($clouds)) {
        ...
        foreach ($cloud in $clouds) {
            ...
            try {
                ...
                $instances = Invoke-RestMethod -Uri https://$($gAccounts["$account"]['endpoint'])$cloudHref/instances -Headers $headers -Method GET -WebSession $webSessions["$account"]
            } catch {
            ...
            if (!$instances) {
                ...
                foreach ($instance in $instances) {
                    ...
                    $object | Add-Member -MemberType NoteProperty -Name "Account_Endpoint" -Value $($gAccounts["$account"]['endpoint'])
                    $object | Add-Member -MemberType NoteProperty -Name "Cloud_Name" -Value $cloudName
                    $object | Add-Member -MemberType NoteProperty -Name "Cloud_Href" -Value $cloudHref
                    $object | Add-Member -MemberType NoteProperty -Name "Instance_Name" -Value $instance.name
                    $object | Add-Member -MemberType NoteProperty -Name "Instance_Href" -Value $instanceHref
                    $object | Add-Member -MemberType NoteProperty -Name "Resource_UID" -Value $instance.resource_uid
